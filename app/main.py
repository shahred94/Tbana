...
async def prevent_dashboard_cache(request: Request, call_next):
    """Always serve the installed dashboard and overlay instead of stale UI."""

    response = await call_next(request)
    if request.url.path.startswith(("/dashboard/", "/overlay/")):
...
async def startup_event():
    ...
    if (
        ...
        print(
            "TikTok listener not started: login is required."
        )
