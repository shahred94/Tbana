"""Windows launcher for the packaged LiveTrigger desktop application."""
...
def _data_root() -> Path:
    ...
    if local_app_data:
        return Path(local_app_data) / "LiveTrigger"
    return Path.home() / "AppData" / "Local" / "LiveTrigger"
...
def main() -> None:
    ...
    if port_is_open():
        if live_trigger_is_ready():
            ...
            messagebox.showinfo(
                "LiveTrigger",
                "LiveTrigger is already running. The dashboard has been opened.",
            )
        else:
            messagebox.showerror(
                "LiveTrigger",
                "Port 8000 is being used by another application.\n\n"
                "Close that application and start LiveTrigger again.",
            )
        return

    root = tk.Tk()
    root.title("LiveTrigger")
    ...
    tk.Label(
        container,
        text="Keep this window open while using LiveTrigger.",
        bg=panel,
        fg=muted,
        font=("Segoe UI", 9),
        ...
        try:
            ...
            root.after(
                ...
                lambda: status_text.set(
                    f"Unable to start LiveTrigger: {error_message}"
                ),
            )
